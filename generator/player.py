...
def play_audio(audiofile_path, find_in_audio_files_path = True):
    ...
    if find_in_audio_files_path:
        existed = config.get_pathes_to_audio_files_in_system()
        if len(existed) == 0:
    ...
    dst = os_proxy.join(ctx.Get().getConfig().abs_audio_files_path_in_test, os_proxy.basename(audiofile_path))
    ...
    audiofile_path = os_proxy.basename(audiofile_path)
    logging.debug(f"play: {audiofile_path} {rel_path}")
    _play_audio(os_proxy.join(rel_path, audiofile_path))
