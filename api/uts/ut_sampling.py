...
def test_find_start_end_regexes():
    ...
    regexes = sampling.find_start_end_regexes("api/uts/data/sampling/test_log.txt", start_regex, end_regex, 0, -1)
...
def test_extract_samples():
    recording_start_date = sampling.get_recording_start_date("api/uts/data/sampling/test_audio.pcm", "api/uts/data/sampling/test_audio.pcm.date")
    assert datetime.datetime.strptime("2021-12-22 18:32:54.932014", sampling.DATE_FORMAT) == recording_start_date
    start_regex = "DialogUXStateAggregator:executeSetState:from=THINKING,to=SPEAKING,validTransition=true"
    end_regex = "DialogUXStateAggregator:executeSetState:from=SPEAKING,to=IDLE,validTransition=true"
    regexes = sampling.find_start_end_regexes("api/uts/data/sampling/test_log.txt", start_regex, end_regex, 0, -1)
    samples = sampling.extract_samples(recording_start_date, regexes, "api/uts/data/sampling/test_audio.wav", "/tmp/", sample_format = "wav")
    import hashlib
    ...
    sample = samples[0]
    assert checksum(sample) == checksum("api/uts/data/sampling/sample_0.wav")
    sample = samples[1]
    assert checksum(sample) == checksum("api/uts/data/sampling/sample_1.wav")
